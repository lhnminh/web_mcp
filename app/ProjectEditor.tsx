...
import { FormEvent, KeyboardEvent, MouseEvent, PointerEvent, ReactNode, useEffect, useRef, useState } from 'react';
...
function ArchitecturePropertiesPanel({ architecture, selectedWall, selectedOpening, drawingWall, loading, onDrawingWall, onPreviewApartment, onPreviewWall, onPreviewOpening, onResizeApartment, onUpdateWall, onDeleteWall, onRemoveExteriorCorner, onAddDoor, onAddWindow, onUpdateOpening, onDeleteOpening, onCloseOpening }: { architecture: ArchitecturalElement[]; selectedWall?: WallElement; selectedOpening?: OpeningElement; drawingWall: boolean; loading: boolean; onDrawingWall: (drawing: boolean) => void; onPreviewApartment: (width: number, depth: number, height: number) => void; onPreviewWall: (id: string, patch: Partial<Pick<WallElement, 'start' | 'end' | 'thickness' | 'height'>>) => void; onPreviewOpening: (id: string, patch: Partial<Pick<OpeningElement, 'offset' | 'width' | 'height' | 'sillHeight' | 'swing' | 'swingSide'>>) => void; onResizeApartment: (width: number, depth: number, height: number) => Promise<string | null>; onUpdateWall: (id: string, patch: Partial<Pick<WallElement, 'start' | 'end' | 'thickness' | 'height'>>) => Promise<unknown>; onDeleteWall: (id: string) => Promise<unknown>; onRemoveExteriorCorner: (id: string, endpoint: WallEndpoint) => Promise<unknown>; onAddDoor: (wallId: string) => Promise<unknown>; onAddWindow: (wallId: string) => Promise<unknown>; onUpdateOpening: (id: string, patch: Partial<Pick<OpeningElement, 'offset' | 'width' | 'height' | 'sillHeight' | 'swing' | 'swingSide'>>) => Promise<unknown>; onDeleteOpening: (id: string) => Promise<unknown>; onCloseOpening: () => void }) {
  const bounds = getArchitectureBounds(architecture);
  const walls = architecture.filter((element): element is WallElement => element.kind === 'wall');
  ...
  const description = selectedOpening ? selectedWindow ? 'Drag the window or edit its dimensions below.' : 'Drag the door or edit its dimensions and swing below.'
    : selectedWall ? exterior ? 'Right-click the wall to add a corner, or drag it to reshape the perimeter.' : 'Drag the wall or edit its dimensions below.'
    ...
    <aside className="add-object-panel architecture-properties">
      ...
      {selectedOpening && selectedWall ? (
        ...
        <form onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="wall-drag-hint"><span>{exterior ? '⌗' : '↔'}</span><p><strong>{exterior ? 'Right-click to add a corner' : 'Drag wall or either endpoint'}</strong>{exterior ? 'Choose the exact point on the exterior wall, then drag the new corner to reshape it.' : 'Measurements update live and save when released.'}</p></div>
          ...
          {exterior && <div className="exterior-corner-actions"><button type="button" onClick={() => onRemoveExteriorCorner(selectedWall.id, 'start')}>− Start corner</button><button type="button" onClick={() => onRemoveExteriorCorner(selectedWall.id, 'end')}>− End corner</button></div>}
...
function ArchitecturePlanLayer({ architecture, bounds, editMode, selectedWallId, selectedOpeningId, selectedRoomId, drawingWall, onSelectWall, onSelectOpening, onSelectRoom, onAddWall, onAddExteriorCorner, onUpdateWall, onUpdateOpening }: { architecture: ArchitecturalElement[]; bounds: ReturnType<typeof getArchitectureBounds>; editMode: EditMode; selectedWallId: string; selectedOpeningId: string; selectedRoomId: string; drawingWall: boolean; onSelectWall: (id: string) => void; onSelectOpening: (id: string, wallId: string) => void; onSelectRoom: (id: string) => void; onAddWall: (start: Point2, end: Point2) => Promise<unknown>; onAddExteriorCorner: (id: string, offsetMeters?: number) => Promise<unknown>; onUpdateWall: (id: string, patch: Partial<Pick<WallElement, 'start' | 'end'>>) => Promise<unknown>; onUpdateOpening: (id: string, patch: Partial<Pick<OpeningElement, 'offset'>>) => Promise<unknown> }) {
  ...
  const beginWallEndpointDrag = (event: PointerEvent<SVGCircleElement>, wall: WallElement, endpoint: WallEndpoint) => {
    if (drawingWall || event.button !== 0) return;
    event.preventDefault();
  ...
  const beginWallEdgeDrag = (event: PointerEvent<SVGLineElement>, wall: WallElement) => {
    if (drawingWall || event.button !== 0 || selectedWallId !== wall.id) return;
    event.preventDefault();
    event.stopPropagation();
    event.currentTarget.setPointerCapture(event.pointerId);
    const pointerStart = pointFromEvent(event);
  ...
  const addExteriorCornerAtPoint = (event: MouseEvent<SVGLineElement>, wall: WallElement) => {
    if (drawingWall || editMode !== 'architecture' || !isExteriorWall(wall, bounds, architecture)) return;
    event.preventDefault();
    event.stopPropagation();
    ...
    const dy = wall.end.y - wall.start.y;
    const length = wallLength(wall);
    const ratio = Math.max(0, Math.min(1, ((point.x - wall.start.x) * dx + (point.y - wall.start.y) * dy) / (length * length)));
    const offsetMeters = Math.round(ratio * length * 100) / 100;
    void onAddExteriorCorner(wall.id, offsetMeters);
  };
