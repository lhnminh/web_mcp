...
export function addWallCommand(scene: SceneDocument, start: Point2, end: Point2, options: { thickness?: number; height?: number; createId?: (prefix: string) => string } = {}): ArchitectureCommandResult {
  ...
  if (!Number.isFinite(thickness) || thickness < 0.05 || thickness > 0.5 || !Number.isFinite(height) || height < 1.8 || height > 6) return failure('INVALID_INPUT', 'Wall thickness must be 0.05–0.50 m and height must be 1.80–6.00 m.');
...
export function updateWallCommand(scene: SceneDocument, wallId: string, patch: WallPatch): ArchitectureCommandResult {
  ...
  if (nextWall.thickness < 0.05 || nextWall.thickness > 0.5 || nextWall.height < 1.8 || nextWall.height > 6) return failure('INVALID_INPUT', 'Wall thickness must be 0.05–0.50 m and height must be 1.80–6.00 m.');
  const bounds = getArchitectureBounds(scene.architecture);
...
const validateOpening = (scene: SceneDocument, opening: OpeningElement, excludingId?: string): Extract<ArchitectureCommandResult, { ok: false }> | null => {
  ...
  if (opening.width < 0.5 || opening.width > 3 || opening.offset < 0.1 || opening.offset + opening.width > wallLength(wall) - 0.1) return { ok: false, code: 'OPENING_DOES_NOT_FIT', message: 'Openings must be 0.50–3.00 m wide and remain at least 0.10 m from wall corners.' };
